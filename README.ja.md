...
## 概要
このアプリケーションは、1891年から1921年にかけて発行された「Lagos Weekly Record」というナイジェリアの歴史的新聞内における地理的言及パターンとキーワード分析を行うための可視化・分析ツールです。歴史研究者や言語学者、デジタルヒューマニティーズの専門家向けにデザインされています。
...
| `text` | 記事の本文テキスト | 必須 |
| `Year` / `year` | 発行年（数値） | 必須 |
| `Publication Date` / `Publication Date ` / `publication_date` / `publication date` / `Pub Date` / `pubdate` | 発行日（いずれの表記でも受け付けます） | 任意 |
| `no` または `id` | 記事ID | 任意 |
...
|---------|------|------|
| `placename` | 地名 | 必須 |
| `latitude` | 緯度（小数点形式、**-90〜90 の範囲**、例: 6.4550） | 必須 |
| `longitude` | 経度（小数点形式、**-180〜180 の範囲**、例: 3.3841） | 必須 |

> **注意**: カラム名は大文字小文字を区別せずに認識されますが、正確に上記の名前を使用することを推奨します。地名が空、または座標が範囲外の行はスキップされ、スキップされた件数がアップロード時のステータスメッセージに表示されます。

> **方法論上の注意（民族名・集団名の扱い）**: サンプル地名データには
> "Yoruba"、"Egba"、"Ijebu" など、地名としてだけでなく民族・言語・集団名として
> 用いられることの多い語が座標付きで含まれています。本ツールは地名CSVに
> 登録された語をすべて「1つの座標点」として集計するため、紙面上で民族名として
> 使われた言及も地図上の1地点にカウントされます。分析結果を解釈する際、
> またご自身の地名CSVを作成する際は、この点にご注意ください。こうした語を
> 集計から除きたい場合は、地名CSVに含めないでください。
...
### オープンソースライブラリ
このツールは以下のオープンソースライブラリを利用しています（`vendor/` ディレクトリに同梱しているため、地図タイルの取得を除きオフラインでも動作します）：
