...
## 💻 PCにダウンロードして使う（オフライン利用）

インターネット接続なしで使いたい場合や、手元のPCで動かしたい場合は、以下の手順で利用できます。**プログラムのインストールや追加ソフトは不要です。**

1. このページ上部の緑色の **「Code」ボタン → 「Download ZIP」** をクリックしてダウンロード
2. ZIPファイルを展開（解凍）する
3. 展開したフォルダの中の **`index.html` をダブルクリック**してブラウザで開く

> **重要:** `index.html` は同じフォルダ内の `style.css`・`app.js`・`vendor/` を読み込んで動作します。
> **`index.html` だけを単独でコピーしたりメールで送ったりすると動きません。** 必ずフォルダごと扱ってください。
> なお、オフラインでは地図の背景タイル（OpenStreetMap）のみ表示されませんが、分析機能はすべて動作します。

### ファイル構成

| ファイル / フォルダ | 役割 |
|---|---|
| `index.html` | 画面の骨組み（これを開く） |
| `style.css` | 見た目・デザイン |
| `app.js` | 分析・地図・グラフのプログラム本体 |
| `vendor/` | 地図表示（Leaflet）・CSV解析（PapaParse）ライブラリ |
| `sample_analysis.csv` / `sample_locations.csv` | 動作確認用サンプルデータ |
| `tests/` | 開発者向けの自動テスト（利用には不要） |
