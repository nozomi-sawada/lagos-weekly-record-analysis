...
// Sanitize HTML but allow only <strong> tags for keyword highlighting
function sanitizeQuoteHTML(s) {
    // First escape all HTML
    let escaped = escapeHTML(s);
    // Then restore only <strong> and </strong> tags
...
function highlightTermsSafe(rawText, terms) {
    ...
    return escaped.replace(new RegExp(pattern, 'gi'), m => `<strong>${escapeHTML(m)}</strong>`);
}

// Look up a value from a CSV row by trying alternative column names.
// Existing keys are kept first so legacy CSVs behave identically.
...
const PUBLICATION_DATE_COLUMNS = [
    "Publication Date",
    "Publication Date ",
    "publication_date",
    "publication date",
...
const translations = {
    en: {
        ...
        legendMedium: "100-150 mentions",
        legendMediumLow: "50-100 mentions",
        legendLow: "25-50 mentions",
        legendVeryLow: "<25 mentions",
        ...
        sampleFilesHeader: "Need sample files to test the application?",
        downloadAnalysisSample: "Sample Analysis CSV",
        downloadLocationSample: "Sample Location CSV",
        locationRowsSkipped: "{count} invalid location row(s) were skipped (missing name or out-of-range coordinates).",
        noValidLocations: "No valid locations found in the uploaded file. Please check that 'placename', 'latitude' (-90 to 90), and 'longitude' (-180 to 180) are present.",
        csvParseWarnings: "CSV parsed with {count} warning(s). Some rows may have been skipped."
    },
    ja: {
        ...
        legendMedium: "100-150回",
        legendMediumLow: "50-100回",
        legendLow: "25-50回",
        legendVeryLow: "25回未満",
        ...
        sampleFilesHeader: "アプリケーションをテストするためのサンプルファイルが必要ですか？",
        downloadAnalysisSample: "サンプル分析用CSV",
        downloadLocationSample: "サンプル地名CSV",
        locationRowsSkipped: "{count}件の無効な位置情報行をスキップしました（地名の欠落、または座標が範囲外）。",
        noValidLocations: "アップロードされたファイルに有効な位置情報がありません。'placename'、'latitude'（-90〜90）、'longitude'（-180〜180）が含まれているか確認してください。",
        csvParseWarnings: "CSVを{count}件の警告付きで解析しました。一部の行がスキップされた可能性があります。"
    }
};
...
function processLocationCSV(csvData) {
    ...
    Papa.parse(csvData, {
        ...
        complete: function(results) {
            ...
            const { locations: parsedLocations, skipped } = processLocationData(results.data);
            const locationCount = Object.keys(parsedLocations).length;

            // Reject empty result so we don't wipe out the default locationCoordinates later.
            if (locationCount === 0) {
                customLocationData = null;
                locationFileUploaded = false;
                document.getElementById('upload-status').textContent =
                    translations[currentLanguage].noValidLocations;
                updateStartButtonVisibility();
                return;
            }

            customLocationData = parsedLocations;

            let statusMsg = translations[currentLanguage].locationFileLoaded +
                (analysisFileUploaded ? ` ${translations[currentLanguage].bothFilesReady}` : "");
            if (skipped > 0) {
                statusMsg += " " + translations[currentLanguage].locationRowsSkipped.replace('{count}', skipped);
            }
            document.getElementById('upload-status').textContent = statusMsg;
...
// Returns { locations, skipped } so the caller can warn the user.
function processLocationData(data) {
    ...
    data.forEach(row => {
        ...
        // Reject missing names or coordinates outside valid Earth ranges.
        if (!placename || !isValidLatitude(latRaw) || !isValidLongitude(lngRaw)) {
            skipped++;
            return;
        }

        locations[placename] = [Number(latRaw), Number(lngRaw)];
    });
...
function startAnalysis() {
    ...
    reader.onerror = function() {
        handleAnalysisFailure(translations[currentLanguage].errorReadingAnalysis);
    };
    reader.readAsText(window.analysisFile);
}

// Roll back to the upload screen and clear the analysis-file flag
// so the user can re-select a file without reloading the page.
function handleAnalysisFailure(message) {
    ...
    document.getElementById('upload-status').textContent = String(message ?? "");
    updateStartButtonVisibility();
}
...
function processCSVData(csvData) {
    ...
    Papa.parse(csvData, {
        ...
        complete: function(results) {
            ...
            // Treat an empty parse as a failure so we don't drop the user into
            // a broken analysis view.
            if (!results.data || results.data.length === 0) {
                handleAnalysisFailure(translations[currentLanguage].failedParseCSV + " " +
                    translations[currentLanguage].noValidLocations);
                return;
            }

            if (results.errors && results.errors.length > 0) {
                console.warn("[DEBUG] PapaParse warnings:", results.errors);
            }
        ...
        error: function(error) {
            ...
            handleAnalysisFailure(translations[currentLanguage].failedParseCSV + " " + error.message);
        }
    });
...
function extractYears() {
    ...
    rawData.forEach(article => {
        const year = findColumnValue(article, YEAR_COLUMNS);
        if (year) {
            yearSet.add(year);
        }
    });
...
function processText(textData) {
    ...
    // [FIXED] Performance improvement: pre-compile regex patterns for all locations
    const locationPatterns = {};
    locationKeywords.forEach(location => {
        locationPatterns[location] = new RegExp(`\\b${escapeRegExp(location)}\\b`, 'gi');
    });
    ...
    textData.forEach(article => {
        ...
        let publicationDate = findColumnValue(article, PUBLICATION_DATE_COLUMNS);
        ...
        for (const location of locationKeywords) {
            ...
            const regex = locationPatterns[location];
            if (!regex) continue;
...
function showLocationDetails(location) {
    ...
    document.getElementById('toggle-quotes-btn').textContent = translations[currentLanguage].showQuotes;
    ...
    if (location.quotes && location.quotes.length > 0) {
        ...
        if (displayQuotes.length > 0) {
            displayQuotes.forEach(quote => {
                ...
                // XSS-safe: Quote is already safely escaped by highlightTermsSafe() with only <strong> tags
...
function extractBestQuotesForMultipleKeywords(keywords, numberOfQuotes = 10) {
    ...
    // [FIXED] Performance improvement: pre-compile regex patterns for all keywords
    ...
    rawData.forEach(article => {
        const text = article.text || "";
        const publicationDate = findColumnValue(article, PUBLICATION_DATE_COLUMNS);
        const year = findColumnValue(article, YEAR_COLUMNS);
...
function getYearlyMentionsForMultipleKeywords(keywords) {
    ...
    const sorted = Object.keys(yearlyMentions)
        .map(year => ({
            year: parseInt(year),
    ...
    return fillMissingYears(sorted);
}

// Fill years with no mentions with zero counts so the time axis is
// continuous (otherwise e.g. 1893 and 1900 would sit side by side).
function fillMissingYears(yearData) {
    if (yearData.length === 0) return yearData;

    const startYear = yearData[0].year;
    const endYear = yearData[yearData.length - 1].year;

    // Guard against corrupt year values producing a huge range
    if (endYear - startYear > 200) return yearData;

    const countsByYear = {};
    yearData.forEach(item => { countsByYear[item.year] = item.count; });

    const filled = [];
    for (let y = startYear; y <= endYear; y++) {
        filled.push({ year: y, count: countsByYear[y] || 0 });
    }
    return filled;
}
...
function updateKeywordResults(keyword, quotes, relatedTerms, themes, yearlyMentions) {
    ...
    timelineSection.innerHTML = `
        <div class="keyword-section-title">${t.mentionsOverTime.replace('{keyword}', escapeHTML(keyword))}</div>
    ...
    themesSection.innerHTML = `
        <div class="keyword-section-title">${t.mainThemesAssociated.replace('{keyword}', escapeHTML(keyword))}</div>
    ...
    themes.slice(0, 6).forEach(theme => {
        ...
        themeCard.innerHTML = `
            ...
            <div class="theme-words">${theme.terms.map(t => escapeHTML(t)).join(', ')}</div>
        `;
    ...
    termsSection.innerHTML = `
        <div class="keyword-section-title">${t.termsFrequentlyAppearing.replace('{keyword}', escapeHTML(keyword))}</div>
    ...
    relatedTerms.slice(0, 25).forEach(term => {
        ...
        termTag.innerHTML = `
            ...
            <span class="term-count">${escapeHTML(term.count)}</span>
        `;
...
function createTimelineChart(containerId, yearData) {
    ...
    const maxCount = Math.max(1, ...yearData.map(d => d.count));
    ...
    // Label only every Nth year: the sidebar is narrow, so labelling
    // every bar makes the years overlap and become unreadable.
    const labelInterval = yearData.length > 12 ? 5 : 1;
    ...
    yearData.forEach(yearItem => {
        ...
        // Exact value shown on hover instead of a label on every bar
        bar.title = `${yearItem.year}: ${yearItem.count}`;

        if (yearItem.year % labelInterval === 0) {
...
function showEnlargedChart(yearData) {
    ...
    try {
        ...
        // Sizing and centering/scrolling are handled by the
        // .enlarged-bar-container CSS (width: max-content inside a
        // horizontally scrollable .enlarged-chart).
        const barContainer = document.createElement('div');
        barContainer.className = 'enlarged-bar-container';
        container.appendChild(barContainer);

        // Show the modal first so the container has a measurable height
        const modal = document.getElementById('chart-modal');
        if (modal) {
            modal.style.display = 'block';
        }

        // Scale bars to the actual rendered height of the bar area
        const maxHeight = Math.max(100, barContainer.clientHeight - 10);
        const scaleFactor = maxHeight / Math.max(1, maxCount);
        ...
        yearData.forEach(yearItem => {
            ...
            // Count label above bar (skip zero years to reduce noise;
            // the exact value is still available via the hover tooltip)
            if (count > 0) {
        ...
        // Start the horizontal scroll at the earliest year
